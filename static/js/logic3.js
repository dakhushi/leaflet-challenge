 ...
 function createFeatures(earthquakeData) {
     ...
     function onEachFeature(feature, layer) {
         layer.bindPopup(
             `<h3>${feature.properties.place}</h3><p><strong>Time:</strong> ${new Date(feature.properties.time)}</p>
     ...
     d3.json(tectonicPlatesUrl).then(function(tectonicdata) {
         let tectonicPlates = L.geoJSON(tectonicdata, {
             style: function(feature) {
                 return {
                     color: "yellow",  // Line color for tectonic plates
                     weight: 2,        // Line thickness
                     fillOpacity: 0    // Ensures no fill/background color
                 };
             }
         }).addTo(myMap);
